        ...
        if (pkg.reasons) {
            for (const reason of pkg.reasons) {
                if (typeof reason === 'string') {
                    ...
                    body += `  - ${reason.header} ([${reason.hash}](https://github.com/GerdaMC/minecraft-launcher-core/commit/${reason.hash}))\n`
                }
            }
        }
    }
...
function writeChangelog(version, packages) {
    ...
    function log(reason) {
        return `- ${reason.header} ([${reason.hash}](https://github.com/GerdaMC/minecraft-launcher-core/commit/${reason.hash}))\n`
    }
